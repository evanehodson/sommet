...
let lastRunnerPos = null;
let trailPoints = null;
let cumDistArrGlobal = null;

const followBtn = document.getElementById('follow-btn');
followBtn.addEventListener('click', () => {
    followEnabled = !followEnabled;
    followBtn.classList.toggle('active', followEnabled);
    if (followEnabled && livePosition) {
        map.easeTo({ center: [livePosition.lon, livePosition.lat], duration: 600 });
    }
});
...
function wsConnect() {
    ...
    ws.onclose = () => {
        setConnStatus('disconnected', 'No signal');
...
function handleLivePosition(pos) {
    livePosition = pos;
    setConnStatus('tracking', 'Tracking');

    moveRunner(pos.lon, pos.lat);
    showRunner();
    updateRadarPulse(pos.lon, pos.lat);
    updateTrailSplit(pos.lon, pos.lat);
    updateProfileRunner(pos.lon, pos.lat);

    if (followEnabled) {
        const easeOpts = { center: [pos.lon, pos.lat], duration: 500 };
        if (pos.heading != null && !isNaN(pos.heading)) {
            easeOpts.bearing = pos.heading;
        }
        map.easeTo(easeOpts);
    }
}
...
// ── Radar Pulse Positioning ─────────────────────────────────

const radarPulse = document.getElementById('radar-pulse');

function updateRadarPulse(lon, lat) {
    const pt = map.project([lon, lat]);
    radarPulse.style.left = pt.x + 'px';
    radarPulse.style.top = pt.y + 'px';
}

map.on('render', () => {
    if (livePosition) {
        updateRadarPulse(livePosition.lon, livePosition.lat);
    }
});
...
map.on('load', async () => {
    ...
    try {
        ...
        map.addLayer({
            id: 'trail-remaining-outline',
            type: 'line',
            source: 'trail-remaining',
            layout: { 'line-join': 'round', 'line-cap': 'round' },
            paint: {
                'line-color': '#ffffff',
                'line-width': [
                    'interpolate', ['exponential', 2], ['zoom'],
                    9, 0.415, 10, 0.83, 11, 1.66, 12, 3.32,
                    13, 6.64, 14, 13.29, 15, 26.58, 16, 53.15, 17, 106.3, 18, 212.6
                ],
                'line-opacity': 1.0,
                'line-blur': 0
            }
        ...
        map.addLayer({
            ...
            paint: {
                ...
                'line-width': [
                    'interpolate', ['exponential', 2], ['zoom'],
                    9, 0.215, 10, 0.43, 11, 0.86, 12, 1.72,
                    13, 3.44, 14, 6.89, 15, 13.78, 16, 27.55, 17, 55.1, 18, 110.2
                ],
                'line-opacity': 1.0,
                'line-blur': 0
            }
        ...
        function updateTrailSplit(lon, lat) {
            ...
            for (let i = 0; i < pathPoints.length; i++) {
                const dx = pathPoints[i].lon - lon;
                const dy = pathPoints[i].lat - lat;
                const sq = dx * dx + dy * dy;
                if (sq < bestSq) { bestSq = sq; bestIdx = i; }
            }

            const completedCoords = pathPoints.slice(0, bestIdx + 1).map(p => [p.lon, p.lat]);
            map.getSource('trail-completed').setData({
                type: 'Feature',
                geometry: { type: 'LineString', coordinates: completedCoords }
            });

            const remainingCoords = pathPoints.slice(bestIdx).map(p => [p.lon, p.lat]);
        ...
        const runnerSource = map.getSource('live-runner');
        ...
        function showRunner() {
            map.setLayoutProperty('live-runner-layer', 'visibility', 'visible');
        }
        ...
        function interpProfileCoords(lon, lat) {
            if (!trailPoints) return null;
            let bestIdx = 0;
            let bestSq = Infinity;
            for (let i = 0; i < trailPoints.length; i++) {
                const dx = trailPoints[i].lon - lon;
                const dy = trailPoints[i].lat - lat;
                const sq = dx * dx + dy * dy;
                if (sq < bestSq) { bestSq = sq; bestIdx = i; }
            }
            return profile[bestIdx] ? profile[bestIdx].dist : null;
        }

        function updateProfileRunner(lon, lat) {
            runnerDist = interpProfileCoords(lon, lat);
            drawProfile();
        }
        ...
        function drawProfile() {
            ...
            // Fill under completed portion
            if (runnerDist !== null && runnerDist > 0) {
                const gradCompleted = ctx.createLinearGradient(0, pad.top, 0, pad.top + ph);
                gradCompleted.addColorStop(0, 'rgba(255, 51, 102, 0.25)');
                gradCompleted.addColorStop(1, 'rgba(255, 51, 102, 0.05)');
                ctx.beginPath();
                ...
                const srX = pad.left + (runnerDist / maxD) * pw;
                ctx.lineTo(srX, pad.top + ph);
                ctx.closePath();
                ctx.fillStyle = gradCompleted;
                ctx.fill();
            }

            // Fill under remaining portion (lighter)
            const gradRemaining = ctx.createLinearGradient(0, pad.top, 0, pad.top + ph);
            gradRemaining.addColorStop(0, 'rgba(200, 200, 200, 0.08)');
            gradRemaining.addColorStop(1, 'rgba(200, 200, 200, 0.01)');
            ctx.beginPath();
            if (runnerDist !== null && runnerDist > 0) {
                const srX = pad.left + (runnerDist / maxD) * pw;
                ctx.moveTo(srX, pad.top + ph);
                let started = false;
                for (let i = 0; i < profile.length; i++) {
            ...
            ctx.closePath();
            ctx.fillStyle = gradRemaining;
            ctx.fill();

            // Full elevation line (remaining white, completed pink)
            if (runnerDist !== null && runnerDist > 0) {
                ctx.beginPath();
                let started = false;
                for (let i = 0; i < profile.length; i++) {
                ...
                for (let i = 0; i < profile.length; i++) {
                    ...
                    const x = pad.left + (dists[i] / maxD) * pw;
                    const y = pad.top + ph - ((elevs[i] - minE) / rangeE) * ph;
                    if (firstRemaining) { ctx.moveTo(x, y); firstRemaining = false; }
                    else ctx.lineTo(x, y);
                }
                ctx.strokeStyle = '#ccc';
                ctx.lineWidth = 1.5;
